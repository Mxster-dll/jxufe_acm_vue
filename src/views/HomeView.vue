...
const SECTION_IDS = ["home", "about", "news"];
...
/** 三个部分的分界（升序、去重） */
const sectionBounds = () => {
  const tops = SECTION_IDS.map((id) => document.getElementById(id))
    .filter(Boolean)
    .map(docTopOf);
...
const snapToSection = (dy) => {
  ...
  if (dy > 0) {
    ...
    // 往上同理；页首之上那一屏是成员墙，由 onWallWheel 的露墙分支负责，这里不接
...
const onWallWheel = (e) => {
  ...
  const raw = e.deltaMode === 1 ? e.deltaY * 16 : e.deltaMode === 2 ? e.deltaY * window.innerHeight : e.deltaY;
  if (maskOut.value) {
