 ...
 *  1. 站内数据里能拿到的难度信号很薄：xCPC 团队奖 122 行**没有任何名次字段**，
 *     只有奖牌档位；有真实名次的只有蓝桥杯个人榜（省内组别名次）与天梯赛（分数）。
 *     拿不到对手强度，就算不出可信的 Elo 期望胜率。
 *  2. 工作区另有一个独立项目 `07_技术项目/Jxufe Rating` 用 TrueSkill 算校内 Rating，
 *     但它**明确不含蓝桥省赛/国赛**（那份数据的理由是「仅奖项无排名」），而蓝桥杯
 *     恰好是站内最大的数据源（17 届 2978 行个人成绩）；它也不含毕业去向/个人荣誉。
...
export const MEDAL_BASE = { gold: 10, silver: 5, bronze: 3 }
...
const TIER_RULES = [
  ['gold', /冠军|特等|一等奖|金奖|金牌|一等|国一|省一|第一名|第\s*1\s*名/],
...
export function describeRecord(record) {
  ...
  const medal = { gold: '金', silver: '银', bronze: '铜' }[record.medal] || record.medal
...
const PILL_MEDAL = { '🥇': 'gold', '🥈': 'silver', '🥉': 'bronze' }
